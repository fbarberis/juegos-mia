...
// Usar las nuevas imágenes de Uma como símbolos del slot
const umaSlotSymbols = [
  '🌹', '💜', '✨', '💎', '🎉', '⭐', '🎊', '🌟'
];
...
export const SlotMachine: React.FC<SlotMachineProps> = ({ onWin, onLose }) => {
  ...
  const spin = () => {
    ...
    const interval = setInterval(() => {
      setSlots([
        umaSlotSymbols[Math.floor(Math.random() * umaSlotSymbols.length)],
        umaSlotSymbols[Math.floor(Math.random() * umaSlotSymbols.length)],
        umaSlotSymbols[Math.floor(Math.random() * umaSlotSymbols.length)]
      ]);
    }, 100);
    ...
    setTimeout(() => {
      ...
      if (hasWon && remainingPrizes > 0) {
        ...
        const winningSymbol = umaSlotSymbols[Math.floor(Math.random() * umaSlotSymbols.length)];
    ...
    <Card className="w-full max-w-md mx-auto bg-gradient-to-br from-purple-900 via-pink-900 to-indigo-900 border-2 border-yellow-400">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-black/30 rounded-lg p-3">
          <h4 className="text-white font-semibold mb-2">Símbolos de Uma:</h4>
          <div className="grid grid-cols-4 gap-2 text-sm text-gray-300 text-center">
            {umaSlotSymbols.map((symbol, index) => (
